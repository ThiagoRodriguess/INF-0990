...
public class Map{
    ...
    /// </summary>
    public int Update(int x_old, int y_old, int x, int y)
    {
    ...
    /// <summary>
    /// Booleana que verifica se a posição está vazia ou contem radiação e permite que o robo transite.
    /// </summary>
    ...
    /// Booleana que verifica se a posição está vazia ou contem radiação e permite que o robo transite.
    /// </summary>
    private bool IsRadioctive(int x, int y){
        return Matriz[x, y] is Radioctive;
    }
    /// <summary>
    /// Responsável por verificar se há radiação ao redor
    /// </summary>

    public int Radiation(int x, int y){
        int radiacao = 0;
        if(x > 0 && Matriz[x - 1, y] is Radioctive){
            radiacao++;
        }
        if(y > 0 && Matriz[x, y - 1] is Radioctive){
            radiacao++;
        }
        if(x < h - 1 && Matriz[x + 1, y] is Radioctive){
            radiacao++;
        }
        if(y < w - 1 && Matriz[x, y + 1] is Radioctive){
            radiacao++;
        }
        return radiacao;
    }
    ...
    public void Print() {
     ...
     /// <summary>
    /// Booleana por checar se as joias do mapa acabaram
    /// </summary>
    public bool IsDone()
    {
        for (int i = 0; i < Matriz.GetLength(0); i++) {
    ...
    private void GenerateRandom()
    {
        ...
        for(int x = 0; x < w/3; x++)
        {
            int xRandom = r.Next(0, w);
            int yRandom = r.Next(0, h);
            this.Insert(new Radioctive(), xRandom, yRandom);
        }
        for(int x = 0; x < w; x++)
        {
            int xRandom = r.Next(0, w);
